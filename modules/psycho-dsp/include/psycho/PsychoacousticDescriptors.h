...
namespace psycho
{
    ...
    inline float calculateGroupDelay(float r, float theta, float freqNormalized)
    {
        ...
        const float omega = 2.0f * kPi * freqNormalized;
    ...
    public:
        ...
        {
            // Find nearest bin
            const int bin = static_cast<int>((theta + kPi) / (2.0f * kPi) * resolution_);
        ...
        std::vector<std::pair<float, float>> findFormantPeaks(float minThreshold = 0.1f) const
        {
            ...
            for (int i = 1; i < resolution_ - 1; ++i)
            {
                ...
                {
                    const float theta = -kPi + (2.0f * kPi) * i / resolution_;
        ...
        void calculatePAD(const std::vector<std::pair<float, float>>& poles)
        {
            ...
            for (int i = 0; i < resolution_; ++i)
            {
                ...
                for (const auto& pole : poles)
                {
                    ...
                    // Gaussian kernel
                    const float diff = wrapAngle(theta - poleTheta);
    ...
    public:
        ...
        static constexpr std::array<VowelTemplate, 5> vowelTemplates{{
            VowelTemplate{"ah", 730.0f, 1090.0f, 2440.0f}, // Open back vowel
            VowelTemplate{"ee", 270.0f, 2290.0f, 3010.0f}, // Close front vowel
            VowelTemplate{"oh", 500.0f, 870.0f, 2250.0f},  // Open rounded vowel
            VowelTemplate{"eh", 530.0f, 1840.0f, 2480.0f}, // Mid front vowel
            VowelTemplate{"oo", 300.0f, 870.0f, 2250.0f}   // Close rounded vowel
        }};
        ...
        static float calculateVowelness(const std::vector<std::pair<float, float>>& poles, float sampleRate)
        {
            ...
            for (int i = 0; i < std::min(3, static_cast<int>(peaks.size())); ++i)
            {
                ...
                const float freq = theta * sampleRate / (2.0f * kPi);
