...
namespace emu
{
    ...
    struct BiquadSection
    {
        ...
        inline float process(float x) noexcept
        {
            ...
            // Sanitize state variables (defense against NaN/Inf corruption)
            // Protects against: extreme coefficients, denormals, corrupt state restore
            if (!std::isfinite(z1)) z1 = 0.0f;
            if (!std::isfinite(z2)) z2 = 0.0f;

            // CRITICAL: EMU Audity 2000 Architecture
            // Per Dave Rossum's patent & hardware analysis: "low precision multipliers
            // combined with controlled hard clipping within the 2nd-order sections
            // (clipping the delay elements instead of the output or accumulator)"
            //
            // This clips the FEEDBACK PATH (z1, z2), not the output.
            // This is what gives EMU filters their characteristic "warmth" vs digital harshness.
            if (sat > 0.0f) {
                const float g = 1.0f + sat * 4.0f;  // AUTHENTIC EMU: 4.0 scaling (per plugin_dev reference)
                z1 = std::tanh(z1 * g);  // Clip delay element 1 (EMU architecture)
                z2 = std::tanh(z2 * g);  // Clip delay element 2 (EMU architecture)
            }
    ...
    inline void poleToBiquad(const PolePair& p, float& a1, float& a2, float& b0, float& b1, float& b2) noexcept
    {
        ...
        // AUTHENTIC EMU: sum-of-abs method (per plugin_dev reference)
    ...
    struct ZPlaneFilter
    {
        ...
        {
            // OPTIMIZATION: Check FIRST before advancing smoothers
            ...
            {
                // FAST PATH: No parameter changes, coefficients are already correct
                // This saves 60-80% of coefficient computation cost in typical use
                return;  // Don't even read current values - they haven't changed
            }
            
            // Advance smoothers only when actively smoothing
            // (Saves unnecessary computation when parameters are stable)
            ...
            // Additional micro-optimization: skip if change is imperceptible
            // (~0.01% threshold = below human perception)
            ...
            {
                return; // Perceptually identical, save the coefficient updates
            }
        ...
        {
            // OPTIMIZATION: Check if we need per-sample smoothing
            ...
            {
                // FAST PATH: Constant drive and mix for entire block (99% of execution time)
            ...
            {
                // SLOW PATH: Per-sample smoothing when parameters are changing
                for (int n = 0; n < num; ++n)
                {
                    // Advance smoothers per-sample for proper 20ms ramps
                    const float drive = driveSmoothing ? driveSmooth.getNextValue() : driveSmooth.getCurrentValue();
                    const float mix   = mixSmoothing   ? mixSmooth.getNextValue()   : mixSmooth.getCurrentValue();
